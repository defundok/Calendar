...
public class CalendarUtils {
    ...
    public static int convert2Integer(Calendar calendar) {
        return Integer.valueOf(calendar.get(YEAR) + formatMonthDate(calendar.get(MONTH) + 1)
                + formatMonthDate(calendar.get(Calendar.DAY_OF_MONTH)));
    }

    public static int convert2Integer(long seconds) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(new Date(seconds * 1000));
        return convert2Integer(calendar);
    }

    public static String formatMonthDate(int num) {
        if (num < 10) {
            return "0" + num;
        }
